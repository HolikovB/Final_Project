...
def DiagonalFixedModJ (φ : AutSL3 R) : Prop :=
  SL3FixedModJ R φ (FieldAutomorphisms.d1SL R) ∧
  SL3FixedModJ R φ (FieldAutomorphisms.d2SL R) ∧
  SL3FixedModJ R φ (FieldAutomorphisms.d3SL R)
...
def SignedTranspositionsFixedModJ (φ : AutSL3 R) : Prop :=
  SL3FixedModJ R φ (FieldAutomorphisms.w1SL R) ∧
  SL3FixedModJ R φ (FieldAutomorphisms.w2SL R)
...
def BasicGeneratorsFixedModJ (φ : AutSL3 R) : Prop :=
  ...
  SL3FixedModJ R φ (FieldAutomorphisms.w1SL R) ∧
  SL3FixedModJ R φ (FieldAutomorphisms.w2SL R) ∧
  SL3FixedModJ R φ (FieldAutomorphisms.x12SL R)

/-- Exact fixation of the six normalized generators. -/
def BasicGeneratorsFixed (φ : AutSL3 R) : Prop :=
  φ (FieldAutomorphisms.d1SL R) = FieldAutomorphisms.d1SL R ∧
  φ (FieldAutomorphisms.d2SL R) = FieldAutomorphisms.d2SL R ∧
  φ (FieldAutomorphisms.d3SL R) = FieldAutomorphisms.d3SL R ∧
  φ (FieldAutomorphisms.w1SL R) = FieldAutomorphisms.w1SL R ∧
  φ (FieldAutomorphisms.w2SL R) = FieldAutomorphisms.w2SL R ∧
  φ (FieldAutomorphisms.x12SL R) = FieldAutomorphisms.x12SL R
...
theorem diagonal_preserved_after_local_change_of_basis
    ...
    ∃ g₁ : GL3 R,
      GL3IsOneModJ R g₁ ∧
      innerAutSL3byGL3 g₁ (φ (FieldAutomorphisms.d1SL R)) = FieldAutomorphisms.d1SL R ∧
      innerAutSL3byGL3 g₁ (φ (FieldAutomorphisms.d2SL R)) = FieldAutomorphisms.d2SL R ∧
      innerAutSL3byGL3 g₁ (φ (FieldAutomorphisms.d3SL R)) = FieldAutomorphisms.d3SL R := by
  let τ1 : Matrix (Fin 3) (Fin 3) R := φ (FieldAutomorphisms.d1SL R)
  let τ2 : Matrix (Fin 3) (Fin 3) R := φ (FieldAutomorphisms.d2SL R)
  let τ3 : Matrix (Fin 3) (Fin 3) R := φ (FieldAutomorphisms.d3SL R)
  let d1m : Matrix (Fin 3) (Fin 3) R := FieldAutomorphisms.d1SL R
  let d2m : Matrix (Fin 3) (Fin 3) R := FieldAutomorphisms.d2SL R
  let d3m : Matrix (Fin 3) (Fin 3) R := FieldAutomorphisms.d3SL R
  let two_inv : R := ⅟2
  ...
  have ht1_sq : τ1 * τ1 = 1 := by
    have h_mul : τ1 * τ1 = ↑(φ (FieldAutomorphisms.d1SL R) * φ (FieldAutomorphisms.d1SL R)) := rfl
    rw [h_mul]
    rw [← map_mul]
    have hd1 : FieldAutomorphisms.d1SL R * FieldAutomorphisms.d1SL R = 1 := by
      apply Subtype.ext
      change FieldAutomorphisms.d1 R * FieldAutomorphisms.d1 R = 1
      dsimp [FieldAutomorphisms.d1]
  ...
  have ht2_sq : τ2 * τ2 = 1 := by
      have h_mul : τ2 * τ2 = ↑(φ (FieldAutomorphisms.d2SL R) * φ (FieldAutomorphisms.d2SL R)) := rfl
      rw [h_mul, ← map_mul]
      have hd2 : FieldAutomorphisms.d2SL R * FieldAutomorphisms.d2SL R = 1 := by
        apply Subtype.ext
        change FieldAutomorphisms.d2 R * FieldAutomorphisms.d2 R = 1
        dsimp [FieldAutomorphisms.d2]
        rw [Matrix.diagonal_mul_diagonal]
        ext i j
        by_cases h : i = j
        · subst h
  ...
  have hd1_t2prime_comm : d1m * τ2_prime = τ2_prime * d1m := by
    have ht1_t2_comm : τ1 * τ2 = τ2 * τ1 := by
      have h_mul1 : τ1 * τ2 = ↑(φ (FieldAutomorphisms.d1SL R) * φ (FieldAutomorphisms.d2SL R)) := rfl
      have h_mul2 : τ2 * τ1 = ↑(φ (FieldAutomorphisms.d2SL R) * φ (FieldAutomorphisms.d1SL R)) := rfl
      rw [h_mul1, h_mul2, ← map_mul, ← map_mul]
      have hd1d2 : FieldAutomorphisms.d1SL R * FieldAutomorphisms.d2SL R =
                   FieldAutomorphisms.d2SL R * FieldAutomorphisms.d1SL R := by
        apply Subtype.ext
        change FieldAutomorphisms.d1 R * FieldAutomorphisms.d2 R = FieldAutomorphisms.d2 R *
        FieldAutomorphisms.d1 R
        dsimp [FieldAutomorphisms.d1, FieldAutomorphisms.d2]
  ...
  have hd1_goal : innerAutSL3byGL3 g1 (φ (FieldAutomorphisms.d1SL R)) = FieldAutomorphisms.d1SL R
  := by
    apply Subtype.ext
    ...
    rw [hV_inv_val]
    rw [Matrix.one_mul]
  have hd2_goal : innerAutSL3byGL3 g1 (φ (FieldAutomorphisms.d2SL R)) = FieldAutomorphisms.d2SL R
  := by
    apply Subtype.ext
  ...
  · have hd3_split : FieldAutomorphisms.d3SL R = FieldAutomorphisms.d1SL R * FieldAutomorphisms.d2SL R
     := by
      apply Subtype.ext
      change FieldAutomorphisms.d3 R = FieldAutomorphisms.d1 R * FieldAutomorphisms.d2 R
      dsimp [FieldAutomorphisms.d1, FieldAutomorphisms.d2, FieldAutomorphisms.d3]
      rw [Matrix.diagonal_mul_diagonal]
      ext i j
      by_cases h : i = j
      · subst h
...
theorem signed_transpositions_preserved_after_local_change_of_basis
    ...
    ∃ g₂ : GL3 R,
      ...
      innerAutSL3byGL3 g₂ (φ (FieldAutomorphisms.d1SL R)) = FieldAutomorphisms.d1SL R ∧
      innerAutSL3byGL3 g₂ (φ (FieldAutomorphisms.d2SL R)) = FieldAutomorphisms.d2SL R ∧
      innerAutSL3byGL3 g₂ (φ (FieldAutomorphisms.d3SL R)) = FieldAutomorphisms.d3SL R ∧
      innerAutSL3byGL3 g₂ (φ (FieldAutomorphisms.w1SL R)) = FieldAutomorphisms.w1SL R ∧
      innerAutSL3byGL3 g₂ (φ (FieldAutomorphisms.w2SL R)) = FieldAutomorphisms.w2SL R := by
  let v1_mat : Matrix (Fin 3) (Fin 3) R := ↑(φ (FieldAutomorphisms.w1SL R))
  let v2_mat : Matrix (Fin 3) (Fin 3) R := ↑(φ (FieldAutomorphisms.w2SL R))
  let π := IsLocalRing.residue R
  -- Extract λ = - v1_mat 0 1 and show it maps to 1 in the residue field.
  have h_map_lam : π (- v1_mat 0 1) = 1 := by
    have hq : π (v1_mat 0 1) = π ((FieldAutomorphisms.w1 R) 0 1) :=
      (Ideal.Quotient.eq (I := J R) (x := v1_mat 0 1) (y := (FieldAutomorphisms.w1 R) 0 1)).mpr
      (hw_mod.1 0 1)
    have hw1_01 : (FieldAutomorphisms.w1 R) 0 1 = -1 := rfl
    rw [hw1_01] at hq
  ...
  have h_map_mu : π (v2_mat 1 2) = 1 := by
    have hq : π (v2_mat 1 2) = π ((FieldAutomorphisms.w2 R) 1 2) :=
      (Ideal.Quotient.eq (I := J R) (x := v2_mat 1 2) (y := (FieldAutomorphisms.w2 R) 1 2)).mpr 
      (hw_mod.2 1 2)
    have hw2_12 : (FieldAutomorphisms.w2 R) 1 2 = 1 := rfl
    rw [hw2_12] at hq
    rw [hq, map_one]
  ...
  have hg2_mod_J : GL3IsOneModJ R g2 := by
    ...
    · simp [Matrix.diagonal_apply_ne _ h, map_zero]
      exact h
  -- Prove fixing of d1
  have hd1_goal : innerAutSL3byGL3 g2 (φ (FieldAutomorphisms.d1SL R)) = FieldAutomorphisms.d1SL R 
  := by
    apply Subtype.ext
    let d1m : Matrix (Fin 3) (Fin 3) R := ↑(FieldAutomorphisms.d1SL R)
    let τ1 : Matrix (Fin 3) (Fin 3) R := ↑(φ (FieldAutomorphisms.d1SL R))
  ...
  -- Prove fixing of d2
  have hd2_goal : innerAutSL3byGL3 g2 (φ (FieldAutomorphisms.d2SL R)) = FieldAutomorphisms.d2SL R 
  := by
    apply Subtype.ext
    let d2m : Matrix (Fin 3) (Fin 3) R := ↑(FieldAutomorphisms.d2SL R)
    let τ2 : Matrix (Fin 3) (Fin 3) R := ↑(φ (FieldAutomorphisms.d2SL R))
    ...
    rw [h_comm, Matrix.mul_assoc, Units.mul_inv g2, Matrix.mul_one]
  -- Prove fixing of d3
  have hd3_goal : innerAutSL3byGL3 g2 (φ (FieldAutomorphisms.d3SL R)) = FieldAutomorphisms.d3SL R 
  := by
    apply Subtype.ext
    let d3m : Matrix (Fin 3) (Fin 3) R := ↑(FieldAutomorphisms.d3SL R)
    let τ3 : Matrix (Fin 3) (Fin 3) R := ↑(φ (FieldAutomorphisms.d3SL R))
    ...
    rw [h_comm, Matrix.mul_assoc, Units.mul_inv g2, Matrix.mul_one]
  -- Prove fixing of w1
  have hw1_goal : innerAutSL3byGL3 g2 (φ (FieldAutomorphisms.w1SL R)) = FieldAutomorphisms.w1SL R 
  := by
    apply Subtype.ext
    let w1m : Matrix (Fin 3) (Fin 3) R := ↑(FieldAutomorphisms.w1SL R)
    ...
    have eq_d3 : v1_mat * d3m = d3m * v1_mat := by
      have h_sl3 : FieldAutomorphisms.w1SL R * FieldAutomorphisms.d3SL R =
      FieldAutomorphisms.d3SL R * FieldAutomorphisms.w1SL R := by
        apply Subtype.ext
        change FieldAutomorphisms.w1 R * FieldAutomorphisms.d3 R =
        FieldAutomorphisms.d3 R * FieldAutomorphisms.w1 R
        ext i j
        dsimp [FieldAutomorphisms.w1, FieldAutomorphisms.d3]
        rw [Matrix.mul_apply, Matrix.mul_apply]
        rw [Fin.sum_univ_three, Fin.sum_univ_three]
        fin_cases i <;> fin_cases j <;> (simp)
      have h_phi_eq := congrArg φ h_sl3
      rw [map_mul, map_mul] at h_phi_eq
      have h_mat_eq : v1_mat * (↑(φ (FieldAutomorphisms.d3SL R)) : Matrix (Fin 3) (Fin 3) R) =
      (↑(φ (FieldAutomorphisms.d3SL R)) : Matrix (Fin 3) (Fin 3) R) * v1_mat :=
        congrArg (fun x : SL3 R => (↑x : Matrix (Fin 3) (Fin 3) R)) h_phi_eq
      have h_phi_d3 : ↑(φ (FieldAutomorphisms.d3SL R)) = d3m := congrArg Subtype.val hdiag_exact.2.2
      rw [h_phi_d3] at h_mat_eq
      exact h_mat_eq
    have eq_d1 : v1_mat * d1m = d2m * v1_mat := by
      have h_sl3 : FieldAutomorphisms.w1SL R * FieldAutomorphisms.d1SL R =
      FieldAutomorphisms.d2SL R * FieldAutomorphisms.w1SL R := by
        apply Subtype.ext
        change FieldAutomorphisms.w1 R * FieldAutomorphisms.d1 R =
        FieldAutomorphisms.d2 R * FieldAutomorphisms.w1 R
        ext i j
        dsimp [FieldAutomorphisms.w1, FieldAutomorphisms.d1, FieldAutomorphisms.d2]
        rw [Matrix.mul_apply, Matrix.mul_apply]
        rw [Fin.sum_univ_three, Fin.sum_univ_three]
        fin_cases i <;> fin_cases j <;> (simp)
      have h_phi_eq := congrArg φ h_sl3
      rw [map_mul, map_mul] at h_phi_eq
      have h_mat_eq : v1_mat * (↑(φ (FieldAutomorphisms.d1SL R)) : Matrix (Fin 3) (Fin 3) R) =
      (↑(φ (FieldAutomorphisms.d2SL R)) : Matrix (Fin 3) (Fin 3) R) * v1_mat :=
        congrArg (fun x : SL3 R => (↑x : Matrix (Fin 3) (Fin 3) R)) h_phi_eq
      have h_phi_d1 : ↑(φ (FieldAutomorphisms.d1SL R)) = d1m := congrArg Subtype.val hdiag_exact.1
      have h_phi_d2 : ↑(φ (FieldAutomorphisms.d2SL R)) = d2m := congrArg Subtype.val hdiag_exact.2.1
      rw [h_phi_d1, h_phi_d2] at h_mat_eq
      exact h_mat_eq
    have eq_sq : v1_mat * v1_mat = d3m := by
      have h_sl3 : FieldAutomorphisms.w1SL R * FieldAutomorphisms.w1SL R =
      FieldAutomorphisms.d3SL R := by
        apply Subtype.ext
        change FieldAutomorphisms.w1 R * FieldAutomorphisms.w1 R = FieldAutomorphisms.d3 R
        ext i j
        dsimp [FieldAutomorphisms.w1, FieldAutomorphisms.d3]
        rw [Matrix.mul_apply]
      ...
      (↑(φ (FieldAutomorphisms.d3SL R)) : Matrix (Fin 3) (Fin 3) R) :=
        congrArg (fun x : SL3 R => (↑x : Matrix (Fin 3) (Fin 3) R)) h_phi_eq
      have h_phi_d3 : ↑(φ (FieldAutomorphisms.d3SL R)) = d3m := congrArg Subtype.val hdiag_exact.2.2
      rw [h_phi_d3] at h_mat_eq
      exact h_mat_eq
    ...
    have hv01_v10 : v1_mat 0 1 * v1_mat 1 0 = -1 := by
      ...
      calc v1_mat 0 1 * v1_mat 1 0 = 0 * 0 + v1_mat 0 1 * v1_mat 1 0 + 0 * v1_mat 2 0 := by ring
           _ = -1 := h
    have h_det : v1_mat.det = 1 := (φ (FieldAutomorphisms.w1SL R)).property
    ...
    · change (0 : R) * v1_mat 0 2 + 0 * v1_mat 1 2 + ↑mu_u * v1_mat 2 2 =
      ...
      rw [hv22]; ring
  -- Prove fixing of w2
  have hw2_goal : innerAutSL3byGL3 g2 (φ (FieldAutomorphisms.w2SL R)) =
  FieldAutomorphisms.w2SL R := by
    apply Subtype.ext
    let w2m : Matrix (Fin 3) (Fin 3) R := ↑(FieldAutomorphisms.w2SL R)
    ...
    have eq_d1_v2 : v2_mat * d1m = d1m * v2_mat := by
      have h_sl3 : FieldAutomorphisms.w2SL R * FieldAutomorphisms.d1SL R =
      FieldAutomorphisms.d1SL R * FieldAutomorphisms.w2SL R := by
        apply Subtype.ext
        change FieldAutomorphisms.w2 R * FieldAutomorphisms.d1 R =
        FieldAutomorphisms.d1 R * FieldAutomorphisms.w2 R
        ext i j
        dsimp [FieldAutomorphisms.w2, FieldAutomorphisms.d1]
        rw [Matrix.mul_apply, Matrix.mul_apply]
        rw [Fin.sum_univ_three, Fin.sum_univ_three]
        fin_cases i <;> fin_cases j <;> (simp)
      have h_phi_eq := congrArg φ h_sl3
      rw [map_mul, map_mul] at h_phi_eq
      have h_mat_eq : v2_mat * (↑(φ (FieldAutomorphisms.d1SL R)) : Matrix (Fin 3) (Fin 3) R) =
      (↑(φ (FieldAutomorphisms.d1SL R)) : Matrix (Fin 3) (Fin 3) R) * v2_mat :=
        congrArg (fun x : SL3 R => (↑x : Matrix (Fin 3) (Fin 3) R)) h_phi_eq
      have h_phi_d1 : ↑(φ (FieldAutomorphisms.d1SL R)) = d1m := congrArg Subtype.val hdiag_exact.1
      rw [h_phi_d1] at h_mat_eq
      exact h_mat_eq
    have eq_d2_v2 : v2_mat * d2m = d3m * v2_mat := by
      have h_sl3 : FieldAutomorphisms.w2SL R * FieldAutomorphisms.d2SL R =
      FieldAutomorphisms.d3SL R * FieldAutomorphisms.w2SL R := by
        apply Subtype.ext
        change FieldAutomorphisms.w2 R * FieldAutomorphisms.d2 R =
        FieldAutomorphisms.d3 R * FieldAutomorphisms.w2 R
        ext i j
        dsimp [FieldAutomorphisms.w2, FieldAutomorphisms.d2, FieldAutomorphisms.d3]
        rw [Matrix.mul_apply, Matrix.mul_apply]
        rw [Fin.sum_univ_three, Fin.sum_univ_three]
        fin_cases i <;> fin_cases j <;> (simp)
      have h_phi_eq := congrArg φ h_sl3
      rw [map_mul, map_mul] at h_phi_eq
      have h_mat_eq : v2_mat * (↑(φ (FieldAutomorphisms.d2SL R)) : Matrix (Fin 3) (Fin 3) R) =
      (↑(φ (FieldAutomorphisms.d3SL R)) : Matrix (Fin 3) (Fin 3) R) * v2_mat :=
        congrArg (fun x : SL3 R => (↑x : Matrix (Fin 3) (Fin 3) R)) h_phi_eq
      have h_phi_d2 : ↑(φ (FieldAutomorphisms.d2SL R)) = d2m := congrArg Subtype.val hdiag_exact.2.1
      have h_phi_d3 : ↑(φ (FieldAutomorphisms.d3SL R)) = d3m := congrArg Subtype.val hdiag_exact.2.2
    ...
    have eq_sq_v2 : v2_mat * v2_mat = d1m := by
      have h_sl3 : FieldAutomorphisms.w2SL R * FieldAutomorphisms.w2SL R =
      FieldAutomorphisms.d1SL R := by
        apply Subtype.ext
        change FieldAutomorphisms.w2 R * FieldAutomorphisms.w2 R = FieldAutomorphisms.d1 R
        ext i j
        dsimp [FieldAutomorphisms.w2, FieldAutomorphisms.d1]
        rw [Matrix.mul_apply]
        rw [Fin.sum_univ_three]
      ...
      (↑(φ (FieldAutomorphisms.d1SL R)) : Matrix (Fin 3) (Fin 3) R) :=
        congrArg (fun x : SL3 R => (↑x : Matrix (Fin 3) (Fin 3) R)) h_phi_eq
      have h_phi_d1 : ↑(φ (FieldAutomorphisms.d1SL R)) = d1m := congrArg Subtype.val hdiag_exact.1
      rw [h_phi_d1] at h_mat_eq
      exact h_mat_eq
    ...
    have hv12_v21 : v2_mat 1 2 * v2_mat 2 1 = -1 := by
      ...
      calc v2_mat 1 2 * v2_mat 2 1 = 0 * v2_mat 0 1 + 0 * 0 + v2_mat 1 2 * v2_mat 2 1 := by ring
           _ = -1 := h
    have h_det : v2_mat.det = 1 := (φ (FieldAutomorphisms.w2SL R)).property
...
theorem transvections_one_preserved_after_local_normalization
    ...
    (hdiag_w_exact :
      ...
      φ (FieldAutomorphisms.w1SL R) = FieldAutomorphisms.w1SL R ∧
      φ (FieldAutomorphisms.w2SL R) = FieldAutomorphisms.w2SL R)
    (hx12_mod : SL3FixedModJ R φ (FieldAutomorphisms.x12SL R)) :
    ∀ i j : Fin 3, ∀ hij : i ≠ j,
...
def IsStandardSL3Aut (φ : AutSL3 R) : Prop :=
  ∃ (σ : R ≃+* R) (ε : Bool) (g : GL3 R),
    ∀ x : SL3 R,
      φ x =
          ((FieldAutomorphisms.graphChoiceSL3 ε)
...
theorem local_class
  ...
  simpa [IsStandardSL3AutNoGraph, IsStandardSL3Aut,
    FieldAutomorphisms.graphChoiceSL3] using hσg x
